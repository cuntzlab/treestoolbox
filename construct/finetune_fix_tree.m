...
%     mDlen = sum(len_tree(handles.referencetree))
    ...
    SeconTermSegs(find(ismember(SeconTermSegs,delSegs))) = [];

    SegLen = [];
    for PrCtr = 1:length(allnewSegs)
        checkNewSegI = find(prvec(NewNodesInd,1) == allnewSegs(PrCtr));
        SegLen(PrCtr) = sum(lenAPtree(NewNodesInd(checkNewSegI)));
    end
    
    Seg2Len = [];
    for PrCtr = 1:length(SeconTermSegs)
        checkNewSegI = find(prvec(NewNodesInd,1) == SeconTermSegs(PrCtr));
    ...
    if isempty(mNBr)
        ...
        while CurrNrBr > mNBr
            EditTree = APReptree;
            [~,SegI] = min(SegLen);
            DelAdd = find(prvec(NewNodesInd,1) == allnewSegs(SegI));      
        ...
        if CurrNrBr > mNBr
            while CurrNrBr > mNBr
                ...
                [~,SegI] = min(Seg2Len);
                DelAdd = find(prvec(NewNodesInd,1) == SeconTermSegs(SegI));
                Seg2Len(SegI) = [];
                SeconTermSegs(SegI) = [];
                DelFromTree = [DelFromTree;NewNodesInd(DelAdd)];
                EditTree = delete_tree(EditTree,DelFromTree);
    ...
    %             plot_tree(EditTree); shine;
        ...
        if isfield(APReptree,'AddedMain')
            ...
            for PrCtr = 1:length(delSegs)
                checkNewSegI = find(prvec(TrueInd,1) == delSegs(PrCtr));
                delSegLen(PrCtr) = sum(lenAPtree(TrueInd(checkNewSegI)));
            end
            while CurrNrBr > mNBr
                EditTree = APReptree;
                [~,SegI] = min(delSegLen);
                DelAdd = find(prvec(TrueInd,1) == delSegs(SegI));
                delSegLen(SegI) = [];
                delSegs(SegI) = [];
...
%% if dend length still too high prune random termination nodes until Dlen equal or smaller
    ...
    if isempty(mDlen)
        ...
        if CurrDLen > mDlen
            ...
            CurrTermPts = NewNodesInd(find(CurrTermPts(NewNodesInd) == 1));
    %         DirParN = idpar_tree(EditTree); 
            ...
            if ~isempty(CurrTermPts)
                while CurrDLen > mDlen
                    ...
                    CurrTermPts(find(CurrTermPts == ChosN)) = [];
                    if isempty(CurrTermPts)
                        break;
                    end
        ...
        if CurrDLen < mDlen
            ...
            CurrNewTermPts = find(ismember(NewNodesInd,CurrTermPts) == 1);
    ...
    %             conVec = conVec*addLenPerIt;
                ...
                CurrNewTermPts(find(CurrNewTermPts == ChosT)) = [];
            end
