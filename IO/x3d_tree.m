...
fwrite (x3d, ['<?xml version="1.0" encoding="UTF-8"?>', char(13), newline], 'char');
fwrite (x3d, ['<X3D >',                                 char(13), newline], 'char');
fwrite (x3d, ['  <head>',                               char(13), newline], 'char');
fwrite (x3d, ['    <meta content=''TREES toolbox tree'' name=''' name '''/>', ...
    char(13), newline], 'char');
fwrite (x3d, ['  </head>',                              char(13), newline], 'char');
fwrite (x3d, ['  <Scene>',                              char(13), newline], 'char');
fwrite (x3d, ['    <Background skyColor=''1 1 1''/>',   char(13), newline], 'char');
...
for ward = 1 : N
    ...
    if ipart (ward)
        if ~contains (options, '-o') % add a sphere at each node
            if ~isnan (rotaxis (ward, 1))
                fwrite (x3d, ['    <Transform rotation = ''' num2str(rotationMatrix (ward, :)), ...
                    ...
                    ''' >', char(13), newline], 'char');
                fwrite (x3d, ['    <Shape>',         char(13), newline], 'char');
                fwrite (x3d, ['      <Cylinder bottom=''false'' height = ''', num2str(heightVector (ward, :)), ...
                    ''' radius=''' num2str(radiusVector (ward, :)), ...
                    ''' side=''true'' solid=''false'' top=''false''/>', char(13), newline], 'char');
                fwrite (x3d, ['      <Appearance>',  char(13), newline], 'char');
                fwrite (x3d, ['        <Material diffuseColor = ''' num2str(color(ward,:)) ''' ', ...
                    ' transparency =  ''0.3'' />',   char(13), newline], 'char');
                fwrite (x3d, ['      </Appearance>', char(13), newline], 'char');
                fwrite (x3d, ['    </Shape>',        char(13), newline], 'char');
                fwrite (x3d, ['    </Transform>',    char(13), newline], 'char');
            end
        else
            if ~isnan (rotaxis (ward, 1))
                fwrite (x3d, ['    <Transform rotation = ''' num2str(rotationMatrix (ward, :)), ...
                ...
                fwrite (x3d, ['        <Material diffuseColor = ''' ...
                    num2str(color (ward, :)) ''' />', char(13), newline], 'char');
                fwrite (x3d, ['      </Appearance>', char(13), newline], 'char');
            ...
            fwrite (x3d, ['      <Sphere radius=''' ...
                num2str(radiusVector (ward, :)) ''' />', char(13), newline], 'char');
            fwrite (x3d, ['      <Appearance>',      char(13), newline], 'char');
            fwrite (x3d, ['        <Material diffuseColor = ''' ...
                num2str(color (ward, :)) ''' />',    char(13), newline], 'char');
            fwrite (x3d, ['      </Appearance>',     char(13), newline], 'char');
        ...
        fwrite (x3d, ['', char(13), newline], 'char');
    end
end
