...
%     subset of nodes to be used for obtaining PCs (NOT IMPLEMENTED YET!)
%     {DEFAULT: ''}
%
% Output
% -------
...
if strfind       (options, '-m3d')
    ...
    for counter  = 1 : 2      % first rotate to x axis
        ...
        rangle (d (1)) = ...
            sign (d (1) - 2.5)  * ...
            sign (mXYZ (d (2))) * ...
            acosd (dot ([1 0 0], mXYZ));
        tree     = rot_tree (tree, rangle);
        d        = fliplr (d); % for next turn rotate around the 2nd axis
    end
    if raxis     > 1
        % if tree should be rotated to axis other than x, do that:
        rangle   = zeros (3, 1);
        % define left hand rotation around corresponding axis:
        rangle (d (raxis - 1)) = sign (raxis - 2.5) * 90;
        tree     = rot_tree (tree, rangle);
    end
