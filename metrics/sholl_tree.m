...
% - dd       ::integer: diameter difference of concentric circles OR
...
% starting and end points of tree segments:
...
for counter      = 1 : length (dd)
    ...
    YP           = [YP; Ys1];
    ZP           = [ZP; Zs1];
    iD           = [iD; counter*ones(length (find (iu1)), 1)];
    % then u2 points:
    iu2          = ~isnan(u2);
    ...
    YP           = [YP; Ys2];
    ZP           = [ZP; Zs2];
    iD           = [iD; (counter * ones (length (find (iu2)), 1))];
    s (counter)  = sum (iu1) + sum (iu2);
    sd (counter) = sum (iu1 &~ iu2);
end
...
if strfind       (options, '-s') % show option
    ...
    for counter  = 1 : length (dd)
        plot     ( ...
            X1 (1) + dd (counter) .* sin (0 : pi / 16 : 2 * pi) / 2, ...
            Y1 (1) + dd (counter) .* cos (0 : pi / 16 : 2 * pi) / 2, ...
            'r-');
    end
    ax           = gca;
    grid         on;
    ...
    HP (2)       = plot (1, 1, 'g-');
    legend       (HP, ...
        {'sphere diameter', 'intersections count'}, ...
        'location', 'SouthWest');
    set          (HP, ...
        'visible',             'off');
    tix          = 0 : round (max (s) / 3) : max (s);
    set          (ax2, ...
        'ytick',               0.9 * dd (end) * tix ./ (2 * max (s)), ...
        'yticklabel',          num2str (tix'));
    HT           = ylabel ('sholl intersections');
    set          (HT, 'color', [0 1 0]);
end
...
if strfind       (options, '-3s') % 3D show option
    ...
    for counter  = 1 : length (dd),
        ...
        p        = patch (surf2patch ( ...
            ...
            Y1 (1) + (YS .* dd (counter) / 2), ...
            Z1 (1) + (ZS .* dd (counter) / 2)));
        set      (p, ...
            'facecolor',       [1 0 0], ...
            'facealpha',       0.01, ...
            'edgecolor',       'none');
